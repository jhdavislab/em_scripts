...
def add_args(parser):
    ...
    parser.add_argument('--Nimg', type=int, default=None, help='Optionally only process the first Nimages')
    return parser
...
def main(args):
    ...
    rot = np.array([x[RKEY] for x in data[:Nimg]])
    ...
    trans = np.array([x[TKEY] for x in data[:Nimg]])
    if args.hetrefine:
